...
local anchor, parent, x, y = unpack(C.MinimapPoint)
...
--================================================--
	...
	Minimap:SetPoint(unpack(C.MinimapPoint))
	...
	Minimap:SetScale(C.MinimapScale)
...
local Stat = CreateFrame("Button", "EKMinimapTooltipButton", Minimap)
	...
	Stat:SetSize(28, 28)
	Stat:ClearAllPoints()
	Stat:SetPoint(iconAnchor and "BOTTOMRIGHT" or "BOTTOMLEFT", Minimap, iconAnchor and 1 or -1, 2)
...
local function createGarrisonTooltip(self)
	if not C.CharacterIcon then return end
	
	GameTooltip:SetOwner(self, "ANCHOR_BOTTOM", iconAnchor and (Minimap:GetWidth()*.7) or -(Minimap:GetWidth()*.7), -10)
...
SlashCmdList["RESETMM"] = function()
	ResetMM()
end
SLASH_RESETMM1 = "/rm"
SLASH_RESETMM2 = "/resetmm"
